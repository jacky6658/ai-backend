...
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")  # 兼容舊名
...
4) 產出欄位：main_copy（主貼文）、alternates（3-4個短開頭）、hashtags（陣列）、cta（短句）。
"""
...
def _ensure_json_block(text: str) -> str:
    ...
    if t.startswith("```"):
        ...
        parts = t.split(fence)
        if len(parts) >= 3:
            t = parts[1] if parts[0].lower().startswith("```json") else parts[1]
    # 找最外層 array or object
    i1 = t.find("{")
    i2 = t.find("[")
...
async def chat_generate(req: Request):
    ...
    body: {
      user_id?: str,
      session_id?: str,         # 前端可忽略；此版僅回傳同值占位
      messages: [{role, content}],
      previous_segments?: [segment...],
      remember?: bool,
      mode?: "script" | "copy"  # 可不傳，會自動判斷
    }
    """
    try:
        data = await req.json()
    ...
    # 太短則回引導（不當錯誤，維持 200 OK）
    hint = SHORT_HINT_COPY if mode == "copy" else SHORT_HINT_SCRIPT
    if len(user_input) < 6:
        return {"session_id": data.get("session_id") or "s", "assistant_message": hint, "segments": [], "copy": None, "error": None}
    ...
    try:
        ...
        return JSONResponse(status_code=500, content={"session_id": data.get("session_id") or "s", "assistant_message": "伺服器忙碌，稍後再試", "segments": [], "copy": None, "error": "internal_server_error"})
